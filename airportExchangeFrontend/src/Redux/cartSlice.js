...
export const cartSlice = createSlice({
			...
			state.total = state.cartItems.reduce((a, b) => {
				return (
					Math.round(
						(a +
							(b.quantity *
								Math.round(b.product_id.price * 100)) /
								100) *
							100,
					) / 100
				)
	...
	extraReducers: {
		...
		[incrementCartItem.pending]: (state) => {
			state.error = false
		},
		...
		[incrementCartItem.rejected]: (state) => {
			state.error = true
		},
		[decrementCartItem.pending]: (state) => {
			state.error = false
		},
		...
		[decrementCartItem.rejected]: (state) => {
			state.error = true
		},
